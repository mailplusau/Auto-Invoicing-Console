...
function invoiceCreation() {
	...
	nlapiLogExecution('AUDIT', 'prev_deployment', ctx.getSetting('SCRIPT', 'custscript_prev_deployment'));
	...
	var error_customers_string = ctx.getSetting('SCRIPT', 'custscript_error_customers');
	var error_special_customers_string = ctx.getSetting('SCRIPT', 'custscript_error_special_customers');
	...
	nlapiLogExecution('AUDIT', 'Customer', ctx.getSetting('SCRIPT', 'custscript_customer_id'));
	nlapiLogExecution('AUDIT', 'Invoice', ctx.getSetting('SCRIPT', 'custscript_invoiceid'));

	if (isNullorEmpty(ctx.getSetting('SCRIPT', 'custscript_customer_id')) && isNullorEmpty(ctx.getSetting('SCRIPT', 'custscript_invoiceid'))) {

		// nlapiLogExecution('DEBUG', 'START ---> ', ctx.getRemainingUsage());

		var searched_summary = nlapiLoadSearch('customrecord_job', 'customsearch_job_inv_process_customer');
		...
		resultSet_summary.forEachResult(function (searchResult_summary) {
			...
			nlapiLogExecution('DEBUG', 'START ---> Customer' + searchResult_summary.getText('custrecord_job_customer', null, 'group'), ctx.getRemainingUsage());

			var customer_internal_id = searchResult_summary.getValue('internalid', 'CUSTRECORD_JOB_CUSTOMER', 'group');
			var special_customer_internal_id = searchResult_summary.getValue("custrecord_job_special_customer", null, "GROUP");
			var customerPO = searchResult_summary.getValue("custentity11", "CUSTRECORD_JOB_CUSTOMER", "GROUP");
			var specialCustomerPO = searchResult_summary.getValue("custentity11", "CUSTRECORD_JOB_SPECIAL_CUSTOMER", "GROUP");
			service_start_date = searchResult_summary.getValue("custrecord_job_date_scheduled", null, "MIN");
			service_end_date = searchResult_summary.getValue("custrecord_job_date_scheduled", null, "MAX");

			nlapiLogExecution('DEBUG', 'special_customer_internal_id', special_customer_internal_id);
			...
			if (result_customer == -1 && result_specialCustomer == -1) {
				...
				*/
				...
				newFilters_job_group_inv_review_descp[newFilters_job_group_inv_review_descp.length] = new nlobjSearchFilter('custrecord_job_customer', null, 'is', customer_internal_id);
				if (!isNullorEmpty(service_start_date) && !isNullorEmpty(service_end_date)) {

					newFilters_job_group_inv_review_descp[newFilters_job_group_inv_review_descp.length] = new nlobjSearchFilter('custrecord_job_date_scheduled', null, 'onorafter', nlapiStringToDate(service_start_date));
					newFilters_job_group_inv_review_descp[newFilters_job_group_inv_review_descp.length] = new nlobjSearchFilter('custrecord_job_date_scheduled', null, 'onorbefore', nlapiStringToDate(service_end_date));
				}
				...
				*/
				try {
					if (result_job_group_inv_review_descp.length == 0) {
						...
						var customer_name = searchResult_summary.getText('custrecord_job_customer', null, 'group');
						var date_reviewed = searchResult_summary.getValue('custrecord_job_date_reviewed', null, 'group');
						// var invoiceable = searchResult_summary.getValue('custrecord_job_invoiceable', null, 'group');
						var date_finalised = searchResult_summary.getValue('custrecord_job_date_inv_finalised', null, 'group');
						var count_jobs_invoiceable = searchResult_summary.getValue('formulanumeric', null, 'sum');
						var zee_text = searchResult_summary.getValue('formulatext', null, 'GROUP');
						// franchisee = searchResult_summary.getValue("custrecord_service_franchisee", "CUSTRECORD_JOB_SERVICE", "GROUP");
						var count_jobs = searchResult_summary.getValue('internalid', null, 'count');
						service_start_date = searchResult_summary.getValue("custrecord_job_date_scheduled", null, "MIN");
						service_end_date = searchResult_summary.getValue("custrecord_job_date_scheduled", null, "MAX");
						...
						newFilters_zee[newFilters_zee.length] = new nlobjSearchFilter('entityid', null, 'is', zee_text);
						...
						if (count_zee == 0) {
							...
							nlapiSendEmail(409635, ['ankith.ravindran@mailplus.com.au', 'willian.suryadharma@mailplus.com.au'], 'Invoice Creation - Empty Zee for Invoicing', body, null);
							return false;
						} else {
							franchisee = zee_id;
						}
						...
						if (!isNullorEmpty(date_reviewed) && count_jobs_invoiceable == 0 && !isNullorEmpty(date_finalised)) {
							...
							if (isNullorEmpty(special_customer_internal_id)) {
								...
								recInvoice.setFieldValue('entity', special_customer_internal_id);
							...
							recInvoice.setFieldValue('custbody_inv_date_range_from', '1/10/2024');
							...
							var searched_singleline_discount_jobs = nlapiLoadSearch('customrecord_job', 'customsearch_job_inv_process_disc_single');
							...
							if (isNullorEmpty(special_customer_internal_id)) {
								newFilters_singleline[newFilters_singleline.length] = new nlobjSearchFilter('custrecord_job_customer', null, 'is', customer_internal_id);
								newFilters_singleline[newFilters_singleline.length] = new nlobjSearchFilter('custrecord_job_special_customer', null, 'is', '@NONE@');
							} else {
								newFilters_singleline[newFilters_singleline.length] = new nlobjSearchFilter('custrecord_job_customer', null, 'is', customer_internal_id);
								newFilters_singleline[newFilters_singleline.length] = new nlobjSearchFilter('custrecord_job_special_customer', null, 'is', special_customer_internal_id);
							}
							newFilters_singleline[newFilters_singleline.length] = new nlobjSearchFilter('formulatext', null, 'is', zee_text).setFormula(strFormula);

							if (!isNullorEmpty(service_start_date) && !isNullorEmpty(service_end_date)) {

								newFilters_singleline[newFilters_singleline.length] = new nlobjSearchFilter('custrecord_job_date_scheduled', null, 'onorafter', nlapiStringToDate(service_start_date));
								newFilters_singleline[newFilters_singleline.length] = new nlobjSearchFilter('custrecord_job_date_scheduled', null, 'onorbefore', nlapiStringToDate(service_end_date));
							}

							searched_singleline_discount_jobs.addFilters(newFilters_singleline);

							var resultSet_singleline_jobs = searched_singleline_discount_jobs.runSearch();
							...
							resultSet_singleline_jobs.forEachResult(function (searchResult_singleline_jobs) {
								...
								var ns_item_package = searchResult_singleline_jobs.getValue('custrecord_service_package_ns_item', 'CUSTRECORD_JOB_SERVICE_PACKAGE', null);
								var package_type = searchResult_singleline_jobs.getValue('custrecord_service_package_type', 'CUSTRECORD_JOB_SERVICE_PACKAGE', null);
								...
								if (package_type != 1) {
									...
									if (!isNullorEmpty(discount_detail)) {
										...
										inv_details_rec.setFieldValue('custrecord57_2', customer_internal_id);
										inv_details_rec.setFieldValue('custrecord56_2', ns_item_package);
										var inv_details_rec_id = nlapiSubmitRecord(inv_details_rec);
									}

									recInvoice.selectNewLineItem('item');
									recInvoice.setCurrentLineItemValue('item', 'item', ns_item_package);
									recInvoice.setCurrentLineItemValue('item', 'quantity', total_qty);
									recInvoice.setCurrentLineItemValue('item', 'rate', service_rate);
							...
							var searched_jobs = nlapiLoadSearch('customrecord_job', 'customsearch_job_inv_process_job_inv_yes');

							var newFilters = new Array();
							if (isNullorEmpty(special_customer_internal_id)) {
								newFilters[newFilters.length] = new nlobjSearchFilter('custrecord_job_customer', null, 'is', customer_internal_id);
								newFilters[newFilters.length] = new nlobjSearchFilter('custrecord_job_special_customer', null, 'is', '@NONE@');
							} else {
								newFilters[newFilters.length] = new nlobjSearchFilter('custrecord_job_customer', null, 'is', customer_internal_id);
								newFilters[newFilters.length] = new nlobjSearchFilter('custrecord_job_special_customer', null, 'is', special_customer_internal_id);
							}
							...
							if (!isNullorEmpty(single_line_packages)) {
								newFilters[newFilters.length] = new nlobjSearchFilter('custrecord_job_service_package', null, 'noneof', single_line_packages);
							}
							if (!isNullorEmpty(service_start_date) && !isNullorEmpty(service_end_date)) {

								newFilters[newFilters.length] = new nlobjSearchFilter('custrecord_job_date_scheduled', null, 'onorafter', nlapiStringToDate(service_start_date));
								newFilters[newFilters.length] = new nlobjSearchFilter('custrecord_job_date_scheduled', null, 'onorbefore', nlapiStringToDate(service_end_date));
							}
							...
							resultSet_jobs.forEachResult(function (searchResult_jobs) {
								...
								var customer_internal_id = (searchResult_jobs.getValue('internalid', 'CUSTRECORD_JOB_CUSTOMER', 'group'));
								var customer_name = searchResult_jobs.getText('custrecord_job_customer', null, 'group');
								//use the internal id of the service iunstead of the service name
								var service_id = (searchResult_jobs.getValue('custrecord_job_service', null, 'group'));
								var service_rate = searchResult_jobs.getValue('custrecord_job_service_price', null, 'group');
								var count_service = (searchResult_jobs.getValue('formulacurrency', null, 'sum'));
								var discount_id = (searchResult_jobs.getValue('formulanumeric', null, 'group'));
								var package_id = (searchResult_jobs.getValue('custrecord_job_service_package', null, 'group'));
								var count_extras = (searchResult_jobs.getValue('custrecord_job_extras_qty', null, 'sum'));
								var invoice_single_line = searchResult_jobs.getValue('custrecord_job_invoice_single_line_item', null, 'group');
								var invoice_detail = searchResult_jobs.getValue('custrecord_job_invoice_detail', null, 'group');
								if (!isNullorEmpty(searchResult_jobs.getValue('custrecord_job_date_finalised', null, 'max'))) {
									date_finalised = searchResult_jobs.getValue('custrecord_job_date_finalised', null, 'max');
								}
								...
								var ns_item_id = service_type_record.getFieldValue('custrecord_service_type_ns_item');
								...
								if (count == 0) {
									...
									if (old_service_id != service_id) {
										...
										recInvoice.setCurrentLineItemValue('item', 'item', old_ns_item_id);
										recInvoice.setCurrentLineItemValue('item', 'quantity', old_total_qty);
										recInvoice.setCurrentLineItemValue('item', 'rate', old_service_rate);

										if (!isNullorEmpty(old_inv_details_rec_id)) {
											item_desc = nlapiLoadRecord('customrecord62', old_inv_details_rec_id);
											recInvoice.setCurrentLineItemValue('item', 'custcol1', old_inv_details_rec_id);
											recInvoice.setCurrentLineItemValue('item', 'custcol1_display', item_desc.getFieldValue('name'));
										}
										recInvoice.commitLineItem('item');
								...
								old_package_id = searchResult_jobs.getValue('custrecord_job_service_package', null, 'group');
								old_service_id = searchResult_jobs.getValue('custrecord_job_service', null, 'group');
								old_ns_item_id = ns_item_id;
								old_service_rate = searchResult_jobs.getValue('custrecord_job_service_price', null, 'group');
								old_inv_details_rec_id = inv_details_rec_id;
								old_invoice_single_line = searchResult_jobs.getValue('custrecord_job_invoice_single_line_item', null, 'group');
							...
							if (ctx.getRemainingUsage() <= usage_threshold_invoice) {
								...
								if (count >= 1) {
									...
									//WS Log:
									nlapiLogExecution('AUDIT', 'Cust #: ' + count_loop_cust + ' | Cust ID: ' + customer_internal_id + ' | INV ID: ' + invoiceId + '.', usage_loopstart_cust - ctx.getRemainingUsage());
							...
							nlapiSendEmail(409635, ['ankith.ravindran@mailplus.com.au', 'willian.suryadharma@mailplus.com.au'], 'Invoice Process - Creation', body, null);
						...
						nlapiSendEmail(409635, ['ankith.ravindran@mailplus.com.au', 'willian.suryadharma@mailplus.com.au'], 'Invoice Creation - Customer: ' + customer_internal_id + ' cannot be Invoiced', body, null);
					}
				} catch (e) {
					...
					nlapiSendEmail(409635, ['ankith.ravindran@mailplus.com.au', 'willian.suryadharma@mailplus.com.au'], 'Invoice Creation - Customer: ' + customer_internal_id + ' cannot create Invoice', message, null);
				}
			}
		...
		service_start_date = ctx.getSetting('SCRIPT', 'custscript_service_start_date');
		...
		var result = updateJobs(ctx.getSetting('SCRIPT', 'custscript_customer_id'), ctx.getSetting('SCRIPT', 'custscript_invoiceid'), service_start_date, service_end_date, franchisee, from_invoice, ctx.getSetting('SCRIPT', 'custscript_special_customer_id'), zee_text);
		nlapiLogExecution('AUDIT', 'Return value 2', result);
		if (result == true && from_invoice != 'Yes') {

			nlapiLogExecution('AUDIT', 'To continue with Next customer after Job Update');
			var params = {
				custscript_prev_deployment: ctx.getDeploymentId(),
				custscript_error_customers: error_customers.join(','),
				custscript_error_special_customers: error_specialCustomers.join(','),
...
function updateJobs(customer_internal_id, invoiceId, service_start_date, service_end_date, franchisee, from_invoice, special_customer_internal_id, zee_text) {
	...
	if (!isNullorEmpty(special_customer_internal_id)) {
		nlapiLogExecution('DEBUG', 'special_customer_internal_id', special_customer_internal_id);
		newFilters_alljobs[newFilters_alljobs.length] = new nlobjSearchFilter('custrecord_job_special_customer', null, 'is', special_customer_internal_id);
	} else {
		newFilters_alljobs[newFilters_alljobs.length] = new nlobjSearchFilter('custrecord_job_special_customer', null, 'is', '@NONE@');
	}
	newFilters_alljobs[newFilters_alljobs.length] = new nlobjSearchFilter('formulatext', null, 'is', zee_text).setFormula(strFormula);
	if (!isNullorEmpty(service_start_date) && !isNullorEmpty(service_end_date)) {
	...
	resultSet_alljobs.forEachResult(function (searchResult_alljobs) {
		...
		try {
			if (ctx.getRemainingUsage() <= usage_threshold) {
				...
				nlapiLogExecution('AUDIT', 'Job Update | Customer', customer_internal_id);
			...
			if (isNullorEmpty(job_record.getFieldValue('custrecord_job_date_invoiced')) && isNullorEmpty(job_record.getFieldValue('custrecord_job_invoice'))) {
				...
				if (from_invoice == 'Yes') {
					...
					if (!isNullorEmpty(job_record.getFieldValue('custrecord_job_date_reviewed')) && !isNullorEmpty(job_record.getFieldValue('custrecord_job_date_inv_finalised'))) {
						job_record.setFieldValue('custrecord_job_invoice', invoiceId);
						job_record.setFieldValue('custrecord_job_date_invoiced', getDate());
				...
				nlapiSendEmail(409635, ['ankith.ravindran@mailplus.com.au', 'willian.suryadharma@mailplus.com.au'], 'Invoice Creation - Customer: ' + customer_internal_id + ' cannot update Job', body, null);
			...
			nlapiSendEmail(409635, ['ankith.ravindran@mailplus.com.au', 'willian.suryadharma@mailplus.com.au'], 'Invoice Creation - Customer: ' + customer_internal_id + ' cannot update Job', message, null);
		}
